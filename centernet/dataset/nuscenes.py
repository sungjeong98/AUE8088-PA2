...
class NuscenesDataset(Dataset):
    CLASSES_NAME = (
        '__back_ground__', 'pedestrian', 'animal', 'car', 'motorcycle',
        'bicycle', 'bus', 'truck', 'construction_vehicle', 'emergency', 'trailer',
        'barrier', 'traffic_cone', 'pushable_pullable', 'debris',
        'bicycle_rack')

    def __init__(self, root='../datasets/nuscenes/', resize_size=(416, 416), mode='train',
                 mean=(0.40789654, 0.44719302, 0.47026115), std=(0.28863828, 0.27408164, 0.27809835)):
        self.root = root
        self.image_folder = opj(self.root, f'{mode}', 'images')
        self.annotation = opj(self.root, f'{mode}', 'labels')
        self.mode = mode

        self.transform = Transform('nuscenes')
        ...
        # with open(opj(self.root, f'{mode}', 'images')) as f:
        #     self.samples = f.read().split()
        self.samples = [f.split('.')[0] for f in os.listdir(self.image_folder) if f.endswith('.png')]
        self.samples.sort()
    ...
    def __getitem__(self, idx):
        ...
        img = Image.open(opj(self.image_folder, f'{sample}.png')).convert('RGB')
        label = self.parse_annotation(opj(self.annotation, f'{sample}.txt'))
        img = np.array(img)
        ...
        hm = np.zeros((15, output_h, output_w), dtype=np.float32)
    ...
    def parse_annotation(self, annotation_path):
        ...
        for line in lines:
            ...
            bbox[0] *= self.resize_size[0]
            bbox[1] *= self.resize_size[1]
            bbox[2] *= self.resize_size[0]
            bbox[3] *= self.resize_size[1]
            bbox[2] += bbox[0]
            bbox[3] += bbox[1]
            boxes.append(bbox)
            labels.append(label)
...
if __name__ == '__main__':
    ds = NuscenesDataset('../../../PASCAL_VOC/VOCdevkit/VOC2012', resize_size=(416, 416))
